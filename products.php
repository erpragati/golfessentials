...
---------------------------------------------------------------------------------------------------------------*/
$type=$brand=$flex=$hand=$player=$head=$bounce=$size=$colour=$loft=$glove=$subtype=$shaft_material=$bags=NULL;
$khali = 0;
$list=array(
  "type" => $type,
  "brand" => $brand,
...
$filter = array('player' , 'type' , 'brand' , 'head' , 'hand' , 'loft' ,'flex' , /*'ShaftMaterial' ,*/ 'Glove', 'bounce' , 'size' , 'Bags' , 'colour');
...
foreach ($filter as $category) {
     ...
     $unselectlink = substr($link, 0, -1);             //   Link for unselect option, generated by excluding current category from query string
#     echo "Unselect Link = ".$unselectlink;
     ...
     if($total[0]!=0) {                                //   If current category is not defined [NULL] in the database, skip displaying the menu
          ...
          echo ucfirst(strtolower($category))."<span class=\"right\">[".$total[0]."]</span>";   //   Total number of results for the current category, displayed in bold
          ...
          while ($row=mysql_fetch_array($query)) {     //   Fetch the list-items from database
               ...
               if($subtotal[0]!=0){                    //   If any list-item has zero results, skip displaying it
                    ...
                    if ($row[0]==$current) {           //   If current list-item is selected, display the deselect image right next to it
                         ...
                         echo "<a href=\"".$link.$category."=".$row[0]."&page=0\" class=\"selected\" rel=\"no-follow\">".ucfirst($row[0])."</a>";
                    }                                  //   Unselect link works by excluding the current category from the query string
                    else{
                         echo "<a href=\"".$link.$category."=".$row[0]."&page=0\" rel=\"nofollow\">".ucwords($row[0])."</a>";
                    }
...
<script src="//ajax.googleapis.com/ajax/libs/jquery/1.8.2/jquery.min.js.gz"></script>
