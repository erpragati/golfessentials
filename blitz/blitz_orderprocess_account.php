...
include '../functions.php';
	...
	<h1>Order Number= <?php echo $oid; ?>, Status: <?php echo $order_row[1]; ?>,<br> Payment Option: <?php echo $order_row[16]; ?></h1>
		...
		<form action="blitz_action.php" method="GET">
			...
			<input type="hidden" value="account" name="payment">
	...
	<br><br><br><br>
	<?php
	$sql="SELECT SubProductID, Quantity, OrderDetailID, CourierName, CourierNumber FROM `orderdetail_account` WHERE OrderID=".$oid;
	...
	while($row=mysql_fetch_array($query)){
				...
				$color=GetProductDetails($subid,'color');
					...
					echo "<tr>";
						...
						echo "<input type=\"hidden\" value=\"account\" name=\"payment\">";
					echo "</tr>";
...
echo "</form><br><br><br><br>";
	...
	<!-- ###################################### Courier ############################### -->
	<?php
		$courier_sql="SELECT SubProductID, Quantity, OrderDetailID, CourierName, CourierNumber FROM `orderdetail_account` WHERE OrderID=".$oid;
	...
	if(isset($_GET['courier_name'])){
		...
		$courier_sql="UPDATE `orderdetail_account` SET `CourierName`='$courier_name',`CourierNumber`='$courier_num' WHERE `OrderDetailID`=$orderdetail_id";
		$courier_query=mysql_query($courier_sql);
	}	
